    ...
    <el-container class="exam_reading" v-loading="page_loading">
        ...
        <el-container>
            <el-header style="background-color:#515b65;line-height: 40px;min-width: 940px;height: 40px"  ref="page_head">
                ...
                <!--复查-->
                <!-- <el-row :span="24" style="height: 40px;background-color: #5e7382" v-show="view_search">
                    <el-form :inline="true" :model="formInline" class="demo-form-inline">
                        <el-form-item label="阅卷老师" >
                            <el-select v-model="search_form.userId" placeholder="选择老师">
                                <el-option  :label="'所有老师'" :value="''"  ></el-option>
                                <el-option  :label="item.userName" :value="item.userPermId" v-for="(item,index) in teacher_select_arr" :key="index"></el-option>
                        ...
                        <el-form-item v-if="isCaptain" label="抽查状态" >
                            <el-select placeholder="选择类型">
                                <el-option label="所有" value="1" ></el-option>
                                <el-option label="未核查" value="2" ></el-option>
                                <el-option label="已核查" value="3" ></el-option>
                                <el-option label="已取未核" value="4" ></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="分数范围" >
                            <el-input-number v-model="search_form.scoreMin"  :min="0" :max="Number(add_data.score)" label="最低分"></el-input-number>
                ...
                <div id="drag" v-drag>
                    ...
                    <div class="drag-handle">
                        ...
                        <el-row>
                            ...
                            <button class="drag-grade-error">0×</button>
                        </el-row>
                        <el-row>
                            <button class="drag-grade-num">0</button>
                            <button class="drag-grade-num">.5</button>
                            <button class="drag-grade-right">{{this.add_data.score}}√</button>
                            <button class="drag-sure" @click="printScore">打分</button>
                        </el-row>
                    </div>
                    <div class="cur-score">当前分数：<span>{{boardStrScore?boardStrScore:0}}</span><span>{{board_score?board_score:0}}</span></div>
                </div>


            </el-main>
            <el-footer style="background-color: #3a4b5f;line-height: 40px" height="40px">
                <el-button size="small" round icon="el-icon-document" type="info" @click="set_problrm">标记为打回卷
                </el-button>
        ...
        <el-dialog
                title="问题卷类型"
                :visible.sync="view_problrm_type"
                width="15%"
                v-dialogDrag>
            ...
            <span slot="footer" class="dialog-footer">
              <el-button @click="view_problrm_type = false">取 消</el-button>
              <el-button type="primary" @click="back_to_tem_teacher">确 定</el-button>
            </span>
        </el-dialog>
...
<script src="./reading_exam.js">
    // import reading from './reading_exam'
    //
    // export default {
    //     ...reading
    // }
